...
export function initializeAccessibility() {
  ...
  const savedHighContrast = sessionStorage.getItem(STORAGE_KEYS.HIGH_CONTRAST_MODE);
  ...
  if (savedTextSize && ['small', 'normal', 'large'].includes(savedTextSize)) {
    ...
    const savedFontSize = sessionStorage.getItem(STORAGE_KEYS.ACCESSIBILITY_FONT_SIZE);
    if (savedFontSize) {
      ...
      if (size >= 12 && size <= 24) {
        ...
        updateCSSVariables(size);
        if (size <= 13) {
          sessionStorage.setItem(STORAGE_KEYS.ACCESSIBILITY_TEXT_SIZE, 'small');
          const newSize = fontSizeMap.small;
          document.documentElement.style.fontSize = `${newSize}px`;
          updateCSSVariables(newSize);
        } else if (size <= 19) {
          sessionStorage.setItem(STORAGE_KEYS.ACCESSIBILITY_TEXT_SIZE, 'normal');
          const newSize = fontSizeMap.normal;
          document.documentElement.style.fontSize = `${newSize}px`;
          updateCSSVariables(newSize);
        } else {
          sessionStorage.setItem(STORAGE_KEYS.ACCESSIBILITY_TEXT_SIZE, 'large');
